...
exports.updateItem = async (req, res) => {
  try {
    ...
    if (service_item !== undefined) { payload.push('service_item = ?'); params.push(service_item); }
    if (interval_km !== undefined) { payload.push('interval_km = ?'); params.push(interval_km); }
    if (interval_months !== undefined) { payload.push('interval_months = ?'); params.push(interval_months); }
    if (last_service_date !== undefined) { payload.push('last_service_date = ?'); params.push(last_service_date); }
    if (last_service_mileage !== undefined) { payload.push('last_service_mileage = ?'); params.push(last_service_mileage); }
    
    payload.push('updated_at = CURRENT_TIMESTAMP');
    params.push(id);

    await db.prepare(`UPDATE maintenance_schedules SET ${payload.join(', ')} WHERE id = ?`).run(...params);
...
exports.initDefaultSchedule = async (req, res) => {
  try {
    ...
    const existing = await db.prepare('SELECT COUNT(*) as count FROM maintenance_schedules WHERE vehicle_vin = ?').get(vin);
    if (existing.count > 0) {
