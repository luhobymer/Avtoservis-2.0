...
const hasBrandSpaceSku = (value) => {
  ...
  return (
    ...
    /\b[A-Z]{1,3}\s+\d{2,4}(?:\s+\d{2,4}){0,3}\b/i.test(line) ||
...
const isNoisyParsedPartName = (value) => {
  ...
  if ((name.match(/\b\d{1,4}\b/g) || []).length >= 3) return true;
  if (/(?:^|\s)(?:ce|re|ss|ee|tu|na|hz|or|at|an)(?:\s|$)/i.test(name)) return true;
  return false;
};
...
const getParsedPartQuality = (item) => {
  ...
  const qty = Number(item?.quantity || 0);
  const strongCyrWords = name.match(/[А-Яа-яІіЇїЄєҐґ]{4,}/g) || [];
  const strongLatWords = name.match(/[A-Za-z]{4,}/g) || [];
  const digitTokens = name.match(/\b\d{1,4}\b/g) || [];
  let score = 0;
...
function parseOcrText(text, ocrData = null) {
  ...
  const dateLikeValueRegex = /\b\d{1,2}[./-]\d{1,2}[./-](?:19|20)\d{2}\b/;
  const offerMetaRegex =
    /\b(?:очид|очіку|ожид|дн[іiя.]|мін[.]?\s*(?:ціна|термін)|купити|в\s*1\s*клік|в\s*один\s*клік|наявн|термін)\b/i;
  ...
  const isDateLikeValue = (value) => dateLikeValueRegex.test(normalizeLine(String(value || '')));
  const stripDateLikeValues = (value) =>
    normalizeLine(String(value || '').replace(new RegExp(dateLikeValueRegex.source, 'gi'), ' '));
  const isOfferMetaLine = (value) => {
    const src = normalizeLine(String(value || ''));
    if (!src) return false;
    if (offerMetaRegex.test(src)) return true;
    if (/^\s*на\s+\d{1,2}[./-]\d{1,2}[./-](?:19|20)\d{2}\s*$/i.test(src)) return true;
    return false;
  };

  const isNoiseLine = (line) => {
    if (!line) return true;
    if (isOfferMetaLine(line)) return true;
  ...
  const getDescriptiveNameScore = (value) => {
    ...
    const cyrWords = src.match(/[А-Яа-яІіЇїЄєҐґ]{4,}/g) || [];
    const latWords = src.match(/[A-Za-z]{4,}/g) || [];
    const digitTokens = src.match(/\b\d{1,4}\b/g) || [];
    let score = src.length;
    score += cyrWords.length * 18;
    score += latWords.length * 6;
    if (partsNameNoiseRegex.test(src)) score -= 22;
    if (digitTokens.length >= 2) score -= 12;
    if (src.split(' ').filter((token) => token.length === 1).length >= 3) score -= 18;
    return score;
  };

  const buildPartNumberSearchVariants = (value) => {
    const raw = normalizeLine(String(value || ''));
    if (!raw) return [];
  ...
  const lineContainsPartNumber = (line, partNumber) => {
    ...
    const srcCompact = src.replace(/[\s./-]+/g, '').toLowerCase();
    return buildPartNumberSearchVariants(partNumber).some((variant) => {
      const compactVariant = variant.replace(/[\s./-]+/g, '');
      return srcLower.includes(variant) || (compactVariant && srcCompact.includes(compactVariant));
    });
  };
  ...
  const extractNearbyDescription = (line, partNumber = '') => {
    ...
    out = out
      ...
      .replace(/Прокладха/gi, 'Прокладка');
    out = trimLeadingNameNoise(trimTrailingNameNoise(out));
    if (!out || isNoiseLine(out)) return '';
    if (isLikelyPartNumberLine(out) || isSkuOnlyLine(out)) return '';
    const cyrWords = out.match(/[А-Яа-яІіЇїЄєҐґ]{4,}/g) || [];
    if (partsNameNoiseRegex.test(out) && cyrWords.length < 2) return '';
    if ((out.match(/\b\d{1,4}\b/g) || []).length >= 3 && cyrWords.length < 2) return '';
    return out;
  };
  ...
  const extractLoosePartNumber = (value) => {
    ...
    const patterns = [
      ...
      /\b[A-Z]{1,3}\s+\d{2,4}(?:\s+\d{2,4}){0,3}\b/i,
      /\b[A-Z0-9]{2,}(?:[./-][A-Z0-9]{2,})+\b/i,
      /\b[A-Z]{1,6}\d{3,}[A-Z0-9]*\b/i,
      /\b0\d{4,}\b/i,
      /\b\d{7,}\b/i,
    ];
    ...
    const scoreCandidate = (cand) => {
      let score = 0;
      const compact = cand.replace(/\s+/g, '');
      if (/[A-Z]/i.test(cand) && /\d/.test(cand)) score += 7;
      else if (/[A-Z]/i.test(cand)) score += 2;
      if (/[./-]/.test(cand)) score += 5;
      if (/\s/.test(cand)) score += 4;
    ...
    const best = filtered.sort((a, b) => scoreCandidate(b) - scoreCandidate(a))[0];
  ...
  const derivePriceQtyFromNearbyLines = (startIndex, partNumber = '') => {
    ...
    const getInlinePriceQty = (line, pn = '') => {
      ...
      if (plausible.length === 1) {
        const looksLikeSingleCodeLine =
          !currencyKeywords.test(line) &&
          !priceKeywords.test(line) &&
          isLikelyPartNumberLine(line) &&
          !hasBrand(line) &&
          !hasStrongDescriptiveText(sanitized);
        if (looksLikeSingleCodeLine) return null;
    ...
    for (let i = startIndex; i < lines.length && i <= startIndex + 4; i += 1) {
      ...
      if (
        ...
        !currencyKeywords.test(line) &&
        !priceKeywords.test(line) &&
        (isLikelyPartNumberLine(line) || (/\d/.test(line) && /[A-Za-z]/.test(line) && !hasStrongDescriptiveText(line)))
      ) {
        continue;
      }
      if (i === startIndex) {
  ...
  const improvePriceQtyFromContext = (item) => {
    ...
    for (let idx = 0; idx < lines.length; idx += 1) {
      ...
      const hasExplicitQtyNearby = (() => {
        for (let probe = idx; probe < lines.length && probe <= idx + 4; probe += 1) {
          if (extractQty(lines[probe])) return true;
        }
        return false;
      })();
      if (qty > 1 && anchorNums.length !== 1 && !hasExplicitQtyNearby) continue;
    ...
    const shouldReplace =
      ...
      (bestCandidate.quantity > currentQty && nearlyEquals(bestCandidate.price, currentPrice, 1.0)) ||
  ...
  const improvePartNameFromContext = (item) => {
    ...
    const anchorIndexes = [];
    for (let idx = 0; idx < lines.length; idx += 1) {
      if (lineContainsPartNumber(lines[idx], partNumber)) {
        anchorIndexes.push(idx);
      }
    }
    if (!anchorIndexes.length) return item;
    ...
    for (const anchorIdx of anchorIndexes) {
      ...
      const candidate = trimLeadingNameNoise(trimTrailingNameNoise(collected.join(' ')));
      const candidateScore = getDescriptiveNameScore(candidate);
      if (candidateScore > bestDescScore) {
        bestDesc = candidate;
        bestDescScore = candidateScore;
      }
    }
  ...
  const recoverPartNumberFromContext = (item) => {
    ...
    if (!name) return item;
    const keywords = (name.match(/[А-Яа-яІіЇїЄєҐґ]{5,}/g) || []).slice(0, 3);
  ...
  const appendMissingExplicitPartNumberItems = (items) => {
    ...
    for (let idx = 0; idx < lines.length; idx += 1) {
      ...
      out.push({
        ...
        quantity: priceQty.quantity || 1,
        part_number: pn,
        purchased_by: 'owner',
      });
      existingPn.add(pnKey);
    }
    return out;
  };

  const appendBrandLedContextItems = (items) => {
    const out = Array.isArray(items) ? items.slice() : [];
    const existingPn = new Set(out.map((item) => normalizePartNumberKey(item?.part_number || '')).filter(Boolean));
    for (let idx = 0; idx < lines.length; idx += 1) {
      const anchor = String(lines[idx] || '');
      const brand = extractBrand(anchor);
      if (!brand) continue;
      if (isOfferMetaLine(anchor)) continue;

      const block = [anchor];
      let blockEnd = idx;
      for (let probe = idx + 1; probe < lines.length && probe <= idx + 5; probe += 1) {
        const probeLine = String(lines[probe] || '');
        if (!probeLine) continue;
        const probeBrand = extractBrand(probeLine);
        if (probe > idx + 1 && probeBrand && probeBrand !== brand && !extractLoosePartNumber(anchor)) break;
        if (/разом|итого|всього/i.test(probeLine)) break;
        block.push(probeLine);
        blockEnd = probe;
      }

      const combined = block.join(' ');
      const pn = extractLoosePartNumber(combined);
      const pnKey = normalizePartNumberKey(pn);
      if (!pn || !pnKey || existingPn.has(pnKey) || isLikelyEngineCode(pn)) continue;

      let bestName = '';
      let bestScore = -Infinity;
      for (const fragment of block) {
        const desc = extractNearbyDescription(fragment, pn);
        if (!desc || isOfferMetaLine(desc)) continue;
        const score = getDescriptiveNameScore(desc);
        if (score > bestScore) {
          bestName = desc;
          bestScore = score;
        }
      ...
      if (!bestName) {
        const stripped = trimLeadingNameNoise(trimTrailingNameNoise(stripKnownOrCandidateBrand(stripDateLikeValues(combined))));
        if (hasStrongDescriptiveText(stripped)) bestName = stripped;
      }
      if (!bestName) bestName = prependBrandToName(pn, brand);
      else bestName = prependBrandToName(bestName, brand);

      const priceQty = derivePriceQtyFromNearbyLines(idx, pn);
      const price = Number.isFinite(priceQty?.price) ? priceQty.price : null;
      const qty = Number.isFinite(priceQty?.quantity) && priceQty.quantity > 0
        ? priceQty.quantity
        : extractQty(combined) || 1;
  ...
  const parseRowLine = (line) => {
    ...
    if (isDateLikeValue(line) || isOfferMetaLine(line)) return null;
  ...
  const pushPart = (name, price, quantity, lineForPartNumber) => {
    ...
    const partNumberSource = normalizeLine(
      [String(lineForPartNumber || '').trim(), cleanedName].filter(Boolean).join(' ')
    );
    ...
    const extractPartNumber = (value) => {
      ...
      const best = extractLoosePartNumber(src);
      return isDateLikeValue(best) ? '' : best;
    };
  ...
  const supplementalCandidates = mergePartsAcrossOcrAttempts([
    normalizedStructured,
    normalizedAnchored,
    normalizedParts,
  ]);
  const supplementedItems = Array.isArray(bestItems) ? bestItems.slice() : [];
  const existingPnKeys = new Set(
    supplementedItems.map((item) => normalizePartNumberKey(item?.part_number || '')).filter(Boolean)
  );
  for (const candidate of supplementalCandidates) {
    if (!candidate) continue;
    const pnKey = normalizePartNumberKey(candidate?.part_number || '');
    if (!pnKey || existingPnKeys.has(pnKey)) continue;
    if (getParsedPartQuality(candidate) < 5) continue;
    existingPnKeys.add(pnKey);
  ...
  const improvedItems = removeSuffixPartNumberDuplicates(
    appendBrandLedContextItems(appendMissingExplicitPartNumberItems(supplementedItems)).map((item) =>
  ...
  const filteredItems = improvedItems.filter((item) => {
    ...
    if (pn && isViscosityLikePartNumber(pn)) {
      return false;
    }
